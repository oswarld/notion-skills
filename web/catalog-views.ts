...
export function skillPage(skill: CatalogSkill): string {
  return page(skill.title, `
    ...
    <div class="workbench">
      <section class="input-panel" aria-labelledby="input-heading"><div class="panel-heading"><h2 id="input-heading">1. 내 내용 넣기</h2><button type="button" class="quiet" id="fill-example">예시 넣기</button></div><p class="helper">입력한 내용은 이 화면에서만 사용해요. AI에 전달할 때는 복사한 요청문을 직접 붙여넣으세요.</p>
        <div id="skill-inputs" data-title="${e(skill.title)}">
          ${skill.inputs.map((input, index) => `<label class="input-field" for="input-${index}">${e(input.label)}${index === 0 ? ' <span class="required-note">필수</span>' : ' <span class="optional-note">선택</span>'}<textarea id="input-${index}" data-label="${e(input.label)}" data-sample="${e(input.sample)}" data-required="${index === 0}" maxlength="12000" rows="${index === 0 ? 7 : 3}" placeholder="${e(input.hint)}" aria-describedby="input-feedback"></textarea></label>`).join("")}
    ...
    <section class="prompt-panel" id="prompt-panel" hidden aria-labelledby="prompt-heading"><h2 id="prompt-heading">2. 복사해서 내 AI에 붙여넣기</h2><p>아래 내용은 AI에게 전달할 요청문입니다. 실제 답변은 평소 쓰는 AI 대화창에서 받아보세요.</p><label for="prepared-prompt">내 요청문</label><textarea id="prepared-prompt" rows="12" readonly></textarea><div class="prompt-actions"><button class="primary" id="copy-prompt" type="button">요청문 복사</button><button class="quiet" id="select-prompt" type="button">전체 선택</button><span id="copy-feedback" role="status" aria-live="polite"></span></div></section>
    <section class="reuse-panel"><h2>마음에 들면 계속 사용하세요.</h2><p>Claude의 스킬 업로드 기능을 쓰는 분은 ZIP 파일로 저장할 수 있어요.</p><a class="secondary" href="/catalog/${skill.id}/download">스킬 ZIP 다운로드 ↓</a><a class="text-link" href="/guide#install">설치 방법 보기 →</a><details><summary>작업 지침 보기</summary><pre id="skill-instructions">${e(skill.instructions)}</pre><a href="/catalog/${skill.id}/source">SKILL.md 원본 받기</a></details></section>`, false, true);
}
